...
const cmds = {
    'fd': {
        fn: (x, state) => { if (state.pen) line(0, 0, x, 0); translate(x, 0); },
        args: 1
    }
};
//     'bk': x => { if (this.pen) line(0, 0, -x, 0); translate(-x, 0); },
...
class Parser {
    constructor(code, idx = 0) {
        this.code = code;
        this.idx = idx;
        this.cmds = [];
    }
    parse() {
        const isValidCmd = cmd => cmds[cmd] !== undefined;
        while (this.remaining) {
            let cmdId = this.nextToken(), cmd = cmds[cmdId], args;
            if (cmdId && cmd) {
                args = Array.from({ length: cmd.args }, () => this.nextToken());
                this.cmds.push({ cmdId, args, cmd });
            }
        }
    ...
    nextToken() {
        let cmd = '';
        const brk = / /;
        while (this.remaining && !brk.test(this.currentChar)) {
    ...
    get currentChar() {
        return this.code.charAt(this.idx);
    }
    get remaining() {
